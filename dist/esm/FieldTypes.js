...
const DisplayBlockForm = ({ children, label }) => {
    ...
    const config = (_a = React.useContext(BlockContext)[data.type]) === null || _a === void 0 ? void 0 : _a.config;
    if (!config)
...
const AddBlock = ({ categories }) => {
    ...
    return _jsxs("div", { children: ["add a:", _jsx("ul", { children: blocks.map(([key, { config }]) => _jsx("li", { children: _jsx("button", { type: "button", onClick: () => listHelpers.addRecord({ type: key }), children: config.label }) }, key)) })] });
};
...
export const block = ({ name, label, types, categories }) => {
    ...
    const blocks = Object.entries(React.useContext(BlockContext)).filter(([, definition]) => (!categories || definition.config.categories.find(s => categories.includes(s)))
    ...
    if (blocks.length === 1) {
        return _jsxs("button", { type: "button", onClick: () => setValue({ type: blocks[0][1].config.type, id: randomId() }), children: ["Add ", blocks[0][1].config.label] });
    }
    return _jsxs("div", { children: ["add a:", _jsx("ul", { children: blocks.map(([key, { config }]) => _jsx("li", { children: _jsx("button", { type: "button", onClick: () => setValue({ type: key, id: randomId() }), children: config.label }) }, key)) })] });
};
