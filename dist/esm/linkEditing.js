...
export function selectionLinkEdit(quill) {
    var _a, _b;
    const range = quill.getSelection();
    if (!range)
        return null;
    const initial = (_b = (_a = quill.getFormat(range)[FlexLink.blotName]) === null || _a === void 0 ? void 0 : _a.target) !== null && _b !== void 0 ? _b : null;
...
export function anchorLinkEdit(quill, anchor) {
    var _a, _b, _c;
    const blot = Quill.find(anchor);
    const index = blot ? quill.getIndex(blot) : 0;
    const length = blot ? blot.length() : 0;
    const initial = (_a = readLinkTarget(anchor)) !== null && _a !== void 0 ? _a : { type: 'external', value: (_b = anchor.getAttribute('href')) !== null && _b !== void 0 ? _b : '' };
...
export function attachLinkClick(quill, open) {
    const onClick = (event) => {
        var _a, _b;
...
// the value).
export function applyLink(quill, edit, { text, target }, href) {
...
export function removeLink(quill, edit) {
    if (edit.length > 0)
        quill.formatText(edit.index, edit.length, FlexLink.blotName, false, 'user');
}
