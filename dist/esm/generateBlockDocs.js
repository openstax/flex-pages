...
function blockSlots(fields) {
    ...
    for (const field of fields) {
        if (field.type === 'blocks') {
            slots.push({ label: field.label, categories: (_a = field.categories) !== null && _a !== void 0 ? _a : [] });
        }
...
export function generateBlockDocs(definitions, options = {}) {
    var _a;
    const blocks = Object.entries(definitions).map(([key, def]) => ({ key, meta: def.fields }));
    const out = [];
    ...
    for (const block of blocks)
        collectTypes(topFields(block.meta), usedTypes);
