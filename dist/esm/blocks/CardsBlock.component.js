...
export function CardsBlock({ data }) {
    ...
    const accentColors = toColorList((_d = findByType(data.value.config, 'accent_colors')) === null || _d === void 0 ? void 0 : _d.value);
    const dividerColors = toColorList((_e = findByType(data.value.config, 'divider_colors')) === null || _e === void 0 ? void 0 : _e.value);
    ...
    return (_jsx("div", { className: cn('content-block-cards', styleClass, present(cardSize) && 'has-size', cardColumns && 'has-columns', present(accentSize) && 'has-accent-size', accentColors && 'has-custom-accent', dividerColors && 'has-custom-divider', isDarkBg && 'dark-card-background'), style: {
            '--card-size': cardSize,
