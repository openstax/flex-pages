...
exports.mapPageNodes = mapPageNodes;
const linkBehavior_js_1 = require("./linkBehavior.js");
...
async function rewriteHtmlContent(html, mappers, parseHtml) {
    ...
    if (linkMapper) {
        for (const a of Array.from(document.querySelectorAll('a[data-flex-link]'))) {
            ...
            if (target)
                jobs.push(Promise.resolve(linkMapper(target)).then((t) => (0, linkBehavior_js_1.writeLinkTarget)(a, t)));
        }
    }
    if (imageMapper) {
...
async function processBlock(node, ctx) {
    ...
    const meta = def === null || def === void 0 ? void 0 : def.config;
    if (!meta)
        return block;
    ...
    const prefetchJob = ctx.mappers.hydratePrefetch && (def === null || def === void 0 ? void 0 : def.prefetch)
        ? Promise.resolve(def.prefetch(block.value))
        : undefined;
    // singular-`field` blocks (e.g. text) store their value directly at node.value
    let processed;
    if (meta.field) {
        processed = { ...block, value: await processField(meta.field, block.value, ctx) };
    }
    else {
        const value = block.value;
        const out = { ...value };
        for (const field of (_a = meta.fields) !== null && _a !== void 0 ? _a : []) {
            out[field.name] = await processField(field, value[field.name], ctx);
        }
        processed = { ...block, value: out };
    }
    return prefetchJob ? { ...processed, prefetched: await prefetchJob } : processed;
}
async function mapPageNodes(nodes, blocks, mappers, parseHtml = nativeParseHtml) {
