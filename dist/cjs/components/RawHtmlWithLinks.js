...
function RawHtmlWithLinks({ html, className, block }) {
    ...
    const resolvedHtml = react_1.default.useMemo(() => {
        ...
        dom.querySelectorAll('a[data-flex-link]').forEach((a) => {
            const target = (0, linkBehavior_1.readLinkTarget)(a);
            if (!target)
                return;
    ...
    const onClick = react_1.default.useCallback((e) => {
        ...
        if (!anchor)
            return;
        const target = (0, linkBehavior_1.readLinkTarget)(anchor);
        if (target)
            (0, linkBehavior_1.handleLinkClick)(e, anchor, target, { routes, actions });
    }, [routes, actions]);
    const Tag = block ? 'div' : 'span';
    return ((0, jsx_runtime_1.jsx)(Tag, { className: className, onClick: onClick, suppressHydrationWarning: true, dangerouslySetInnerHTML: { __html: resolvedHtml } }));
}
