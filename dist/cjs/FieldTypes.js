...
const DisplayBlockForm = ({ children, label }) => {
    ...
    const config = (_a = react_1.default.useContext(BlockContext_1.BlockContext)[data.type]) === null || _a === void 0 ? void 0 : _a.blockConfig;
    if (!config)
...
const block = ({ name, label, types, categories }) => {
    ...
    const blocks = Object.entries(react_1.default.useContext(BlockContext_1.BlockContext)).filter(([, definition]) => (!categories || definition.blockConfig.categories.find(s => categories.includes(s)))
