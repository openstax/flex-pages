...
const DisplayBlockForm = ({ children, label }) => {
    ...
    const config = (_a = react_1.default.useContext(BlockContext_1.BlockContext)[data.type]) === null || _a === void 0 ? void 0 : _a.config;
    if (!config)
...
const AddBlock = ({ categories }) => {
    ...
    const blocks = Object.entries(react_1.default.useContext(BlockContext_1.BlockContext))
        .filter(([, definition]) => definition.config.categories.find(s => categories.includes(s)));
    return (0, jsx_runtime_1.jsxs)("div", { children: ["add a:", (0, jsx_runtime_1.jsx)("ul", { children: blocks.map(([key, { config }]) => (0, jsx_runtime_1.jsx)("li", { children: (0, jsx_runtime_1.jsx)("button", { type: "button", onClick: () => listHelpers.addRecord({ type: key }), children: config.label }) }, key)) })] });
};
...
const block = ({ name, label, types, categories }) => {
    ...
    const blocks = Object.entries(react_1.default.useContext(BlockContext_1.BlockContext)).filter(([, definition]) => (!categories || definition.config.categories.find(s => categories.includes(s)))
    ...
    if (blocks.length === 1) {
        return (0, jsx_runtime_1.jsxs)("button", { type: "button", onClick: () => setValue({ type: blocks[0][1].config.type, id: randomId() }), children: ["Add ", blocks[0][1].config.label] });
    }
    return (0, jsx_runtime_1.jsxs)("div", { children: ["add a:", (0, jsx_runtime_1.jsx)("ul", { children: blocks.map(([key, { config }]) => (0, jsx_runtime_1.jsx)("li", { children: (0, jsx_runtime_1.jsx)("button", { type: "button", onClick: () => setValue({ type: key, id: randomId() }), children: config.label }) }, key)) })] });
};
exports.block = block;
