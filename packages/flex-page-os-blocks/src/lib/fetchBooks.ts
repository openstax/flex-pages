...
export interface BookData {
  // the cnx (archive) UUID, used as the lookup key
  id: string;
  title: string;
  slug: string;
  coverUrl: string;
...
const CMS_BOOKS_URL =
  'https://openstax.org/apps/cms/api/v2/pages/?type=books.Book&fields=cnx_id,title,slug,cover_url&limit=250';
...
let allBooksPromise: Promise<Record<string, BookData>> | undefined;

function loadAllBooks(): Promise<Record<string, BookData>> {
  if (!allBooksPromise) {
    allBooksPromise = fetch(CMS_BOOKS_URL)
      .then((response) => response.json())
      .then((data: { items: RawBook[] }) => {
        const byId: Record<string, BookData> = {};
...
export const fetchBooks: FetchBooks = async (ids) => {
